...
#define RSSI_0 -50.0  // RSSI de referencia a 1 metro
#define PATH_LOSS_EXPONENT 2.0  // Exponente de pérdida de ruta
...
ESP_EVENT_DEFINE_BASE(SCAN_BLE_EVENT);
...
esp_timer_handle_t scan_timer = (esp_timer_handle_t) 100000; //Expresado en ms
...
static double estimate_distance(int rssi){
    ...
    return pow(10.0, (RSSI_0 - rssi) / (10.0 * PATH_LOSS_EXPONENT));
}
...
static void update_ble_devices_list(esp_bd_addr_t bd_addr, int rssi, int estimated_distance){
    //Busca si el dispositivo esta en la lista 
    int index_device = find_device_index(bd_addr);
...
static int find_device_index(esp_bd_addr_t bd_addr){
    ...
    return -1; //El dispositivo no esta presente en la lista
}

//TODO Considerar un mecanismo olvidado de los dispositivos que ya no se detectan
static void forget_undetected_devices(){
    time_t current_time = time(NULL);

    for(int i = 0; i < num_devices; i++){
        if(!ble_detected_devices[i].is_in_room && (current_time - ble_detected_devices[i].last_detection_time) > FORGET_THRESHOLD 
                && !ble_detected_devices[i].detected_in_last_scan){
            remove_device(i);
        }
    }
}
...
static void scan_timer_callback(void *arg){
    ...
    esp_event_post(SCAN_BLE_EVENT, SCAN_BLE_EVENT_DEVICE_FOUND, NULL, 0, portMAX_DELAY);
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (rssi > RSSI_THRESHOLD) {
                double distance = estimate_distance(rssi);
                ESP_LOGI(GATTC_TAG, "Device found: RSSI=%d, Distance=%.2f meters", rssi, distance);
                // Actualiza la lista de dispositivos detectados
                capacity++;
                update_ble_devices_list(scan_result->scan_rst.bda, rssi, distance);
            }
            break;
        case ESP_GAP_SEARCH_INQ_CMPL_EVT:
            int *event_data = &capacity;
            ESP_ERROR_CHECK(esp_event_post(SCAN_BLE_EVENT, SCAN_BLE_EVENT_DEVICE_FOUND, &event_data, sizeof(event_data), portMAX_DELAY));
            break;
        default:
            break;
        }
