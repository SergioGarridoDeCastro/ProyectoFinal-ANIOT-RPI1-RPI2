...
static void wifi_power_save(bool power_save_enable){
    if(power_save_enable){
        ESP_LOGI(TAG, "Activando modo de ahorro de energia WiFi");
    }
    else{
        ESP_LOGI(TAG, "Desactivando modo de ahorro de energia WiFi");
    }
   
    //Configura el modo de ahorro de energia WiFi    
    esp_wifi_set_ps(power_save_enable ? WIFI_PS_MAX_MODEM : WIFI_PS_NONE);
}
...
void init_ap_wifi(void) {
    ESP_LOGI(TAG, "ESP_WIFI_MODE_STA");
    wifi_init_sta();

    // Configura el modo de ahorro de energía WiFi después de la inicialización
